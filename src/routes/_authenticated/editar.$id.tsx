...
import { updateVisit, listVisits } from "@/lib/visits.functions";
...
function EditarPage() {
  ...
  const saveVisit = useServerFn(updateVisit);
  ...
  useEffect(() => {
    if (visits) {
      ...
      if (visit) {
        setActivistName(visit.activist_name || "");
        setVisitedAt(toLocalInputValue(new Date(visit.visited_at)));
        setVoterName(visit.voter_name || "");
        setVoteCount(visit.vote_count || "");
        setNotes(visit.notes || "");
        setNeighborhood(visit.neighborhood || "");
        setLocality(visit.locality || "");
        setAddressNumber(visit.address_number || "");
        setIsUndecided(visit.is_undecided || false);
        setUndecidedDetails(visit.undecided_details || "");

        setChoices({
          president: visit.president_choice || "",
          governor: visit.governor_choice || "",
          federal: visit.federal_choice || "",
          state: visit.state_choice || "",
        });
        setOthers({
          president: visit.president_other || "",
          governor: visit.governor_other || "",
          federal: visit.federal_other || "",
          state: visit.state_other || "",
        });
      }
    }
  }, [visits, id]);
  ...
  const mutation = useMutation({
    mutationFn: (payload: Record<string, string | null>) => saveVisit({ data: { id, ...payload } as never }),
    onSuccess: () => {
  ...
  return (
    <AppShell title="Registrar visita">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        >
          {mutation.isPending ? "Atualizando..." : "Atualizar visita"}
        </Button>
      </form>
    </AppShell>
  );
